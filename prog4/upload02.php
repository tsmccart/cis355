...
if ($result) {
    ...
    $sql = "UPDATE customers SET filename = '$fileName', filetype = '$fileType', "
        . "filesize = '$fileSize', description = '$fileDescription' "
        . "WHERE email = '$username'"; //WHERE CLAUSE CAN NOT BE USED WITH INSERT!!!!!!
...
$i = 0; 
foreach ($pdo->query($sql) as $row) {
    echo ' ... [' . $i++ . '] --- ' . getcwd() . '/' . $row['filename'] . '<br>' . 
    "<img src='uploads/" . $row['filename'] . "' style='width:150px;height:100px;'><br>";
}
echo '<br><br>';
